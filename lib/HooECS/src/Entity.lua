...
function Entity:add(component)
	...
	-- attempt to call hook
    if component.addedToEntity then component:addedToEntity(self) end
    return self
end

function Entity:addTag(tag)
    if self.components[tag] then
        HooECS.debug("Entity: Trying to add Tag'" .. tag .. "', but it's already existing. Please use Entity:set to overwrite a component in an entity.")
    else
		local Cls=HooECS.class(tag)
        self.components[tag] = Cls()
        if self.eventManager then
            self.eventManager:fireEvent(HooECS.ComponentAdded(self, tag))
        end
    end
    return self
end
...
function Entity:addMultipleTags(tagList)
...
function Entity:removeTag(tag)
    if self.components[tag] then
        ...
        HooECS.debug("Entity: Trying to remove nonexisting  tag " .. tag .. " from Entity. Please fix this")
    end
    if self.eventManager then
        self.eventManager:fireEvent(HooECS.ComponentRemoved(self, tag))
    end

...
describe('Engine', function()
    ...
    it(':update(dt) callback is called on engine update', function()
        local c = Component1()
        c.num = 1
        entity:add(c)
        entity.update = function(self, dt)
            local c1 = self:get("Component1")
            c1.num = c1.num + dt
        end
        engine:addEntity(entity)
        assert.are.equal(entity:get("Component1").num, 1)
        engine:update(1)
        assert.are.equal(entity:get("Component1").num, 2)
    end)

    it('Entity:setUpdate() adds an update function to the entity after being added to the engine', function()
        local c = Component1()
        c.num = 1
        entity:add(c)
        engine:addEntity(entity)
        ...
        engine:update(1)
        assert.are.equal(entity:get("Component1").num, 1)
        entity:setUpdate(function(self, dt)
                local c1 = self:get("Component1")
                c1.num = c1.num + dt
            end)
        engine:update(1)
    ...
    it('Entity:setUpdate() removes the entity from the update list if no function is provided', function()
        local c = Component1()
        c.num = 1
        entity:add(c)
        engine:addEntity(entity)
    ...
    it('Entity:getChildren() returns a table containing children or nil if there are none', function()
        engine:addEntity(entity)
        assert.are.equal(entity2:getChildren(), nil)
        entity:setParent(entity2)
        assert.are.equal(entity2:getChildren(), entity2.children)
    end)
end)
